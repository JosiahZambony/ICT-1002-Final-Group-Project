...
void gradient_descent_with_k() {
    ...
    */
    double step_size = 0.001, x1 = 1.0, x2 = 1.0, minimum = 0.0, min_range = -4.5, max_range = 4.5, threshold = 0.00001;
    int dimension = 2, iteration = 0, max_iteration = 10000, h_dimension = dimension * dimension;
    int choice = 1;
    ...
    // printf("Before Copy:\n");
    // printf("prev_x_array[0]: %lf\n", prev_x_array[0]);
    // printf("prev_x_array[1]: %lf\n", prev_x_array[1]);
    ...
    for(int count = 0; count < 10000; count++) {
        ...
        for(int index = 0; index < dimension; index++) {
            if((x_array[index] > max_range) || (x_array[index] < min_range)) {
                ...
                fprintf(fptr, "Boundary has been breached\n");
                exit(1);
            }
        }
